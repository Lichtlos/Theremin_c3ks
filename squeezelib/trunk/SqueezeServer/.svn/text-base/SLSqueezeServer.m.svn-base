...
- (void) requestRemovalOfTrackIds:(NSArray *)trackIds {
...
- (void) requestAddingOfTrackIds:(NSArray *)trackIds {
...
- (void) executeDatabaseQueryForType:(PWDatabaseQueryEntityType)aType usingFilters:(NSArray *)someFilters {
	[_conn scheduleRequest:[[[PWDatabaseQuery alloc] initWithEntityType:aType andFilters:someFilters] autorelease]];
}
...
- (void) requestCompleted:(SLCLIRequest *)request {
	...
	if ([request isKindOfClass:[SLCLIPlayerListRequest class]])
		...
		[_delegate fetchedCurrentPlaylistLength:[results count]];
		if ([results count] == 0) {
			[_delegate fetchedCurrentPlaylist:[NSArray array]];
			return;
		}
		
		SLCLIRequest *songInfoReq = [SLCLISongInfoRequest songInfoRequestWithPathList:results];
		[songInfoReq setUserInfo:request];
		[_conn scheduleRequest:songInfoReq];			
	}
	else if ([request isKindOfClass:[SLCLISongInfoRequest class]]) {
		if ([[request userInfo] isEqual:_currentPlaylistRequest]) {
			[_currentPlaylistRequest release], _currentPlaylistRequest = nil;
			[_delegate fetchedCurrentPlaylist:[request results]];			
		} else if ([[request userInfo] isEqual:_userInfoCurrentSong]) {
			[_delegate fetchedCurrentSong:[[request results] objectAtIndex:0]];
		} else {
			// its a old request - just ignore it.
		}
	}
	...
	else if ([request isKindOfClass:[PWDatabaseQuery class]])
		[_delegate databaseQuery:(PWDatabaseQuery *)request finished:[request results]];
}

- (void) request:(SLCLIRequest *)aRequest partialResult:(id)result {
	if ([aRequest isKindOfClass:[SLCLISongInfoRequest class]] && [[aRequest userInfo] isEqual:_currentPlaylistRequest])
