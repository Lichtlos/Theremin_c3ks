...
+ (id) playerPlaylistControlTrackIdRequestWithPlayer:(SLPlayer *)player andMode:(SLCliPlayerPlaylistControlMode)mode andTrackIds:(NSArray *)trackIds {
...
- (id) initWithPlayer:(SLPlayer *)player andMode:(SLCliPlayerPlaylistControlMode)mode andTrackIds:(NSArray *)trackIds {
...
- (NSString *) _command {
	...
	for (int i = 0; i < [_trackIds count]; i++) {
		if (i != 0) [cmd appendString:@","];
		[cmd appendFormat:@"%@", [_trackIds objectAtIndex:i]];
	}
